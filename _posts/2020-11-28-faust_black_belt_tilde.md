...
So far, we have implemented a somewhat elementary circuit. Now, we can try to implement a one-pole lowpass filter with zero-delay feedback topology. The circuit below is taken from the [book](https://www.native-instruments.com/fileadmin/ni_media/downloads/pdf/VAFilterDesign_2.1.2.pdf?sscid=b1k4_zpwrx) by Zavalishin on virtual analogue filters design.
...
$$ \dot{u_n} = u_n - k1 * v_n - k1 * (v_n + k2 * (u_n - k1 * v_n)) $$

$$ \dot{v_n} = v_n + k2 * (u_n - k1 * v_n) $$
